...
/**
 * 使用此缓存一定要注意线程安全问题。 expireAfterWrite优先于refreshAfterWrite判断 refreshAfterWrite < t < expireAfterWrite
 * 时异步reload。
 *
 * @param <K>
 * @param <V>
...
public abstract class AbstractCache<K, V> {
              ...
              new CacheLoader<K, Optional<V>>() {
                ...
                public Optional<V> load(K k) {
                  if (log.isDebugEnabled()) {
                    log.debug(
                        "同步加载数据 class="
                            + AbstractCache.this.getClass().getSimpleName()
                            + " k="
                            + k);
                  }
                  return loadData(k);
                ...
                public ListenableFuture<Optional<V>> reload(K key, Optional<V> oldValue)
                  ...
                  ListenableFutureTask<Optional<V>> task =
                      ListenableFutureTask.create(() -> loadData(key));
  ...
  private static ScheduledExecutorService scheduledExecutorService =
      Executors.newScheduledThreadPool(1, new NamedThreadFactory("缓存状态监控"));
  ...
  /** 获取缓存的对象。 注意对象可能被多个线程修改，造成错误。 可以通过重写此函数拷贝一个新的对象避免此类问题。 */
  public V get(K k) {
    Optional<V> optional;
    try {
  ...
  private void monitor() {
    ...
    log.info(
        "缓存名：{}，缓存数量：{}，缓存命中率：{}，缓存平均加载时间：{}ms，缓存淘汰总数：{},缓存加载成功数：{}，缓存加载失败率：{}",
        this.getClass().getSimpleName(),
        this.loadingCache.size(),
        stats.hitRate(),
