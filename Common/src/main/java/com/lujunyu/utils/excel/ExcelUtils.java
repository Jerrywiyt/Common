...
public class ExcelUtils {
  ...
  public static <T> Workbook getWorkBook(List<T> list) throws Exception {
    ...
    if (list == null || list.size() == 0) return wb;
   ...
   * @throws Exception
   */
  public static <T> List<T> readFromStream(InputStream in, Class<T> clazz, boolean xlsx)
      throws Exception {
   ...
   * @throws Exception
   */
  private static <T> void readCellContent(String key, Field[] fields, Cell cell, T t, DateFormat df)
      throws Exception {
    Object o = null;
    try {
      ...
      if (cell != null) {
        throw new RuntimeException(
            String.format("%s行%s列数据格式不正确", cell.getRowIndex() + 1, cell.getColumnIndex() + 1));
      }
      throw ex;
    }
  }
