...
public class TestCircuitBreaker {
  ...
  public void testCircuitConfig() {
    ...
    CircuitBreakerRegistry circuitBreakerRegistry =
        CircuitBreakerRegistry.ofDefaults(); // 使用默认配置创建注册器
  ...
  public void testCircuit() {
    ...
    CheckedFunction0<String> decoratedService =
        CircuitBreaker.decorateCheckedSupplier(breaker, checkedSupplier);
  ...
  public void test() {
    ...
    CheckedFunction1<Boolean, String> checkedFunction1 =
        CheckedFunction1.of((b) -> new Service1().doSomething(b));
    test.getEventPublisher().onEvent(event -> log.info(event.getEventType() + ""));
    for (int i = 0; i < 11; i++) {
      System.out.println(
          Try.of(() -> CircuitBreaker.decorateCheckedFunction(test, checkedFunction1).apply(true))
              .recover(t -> "方法降级")
              .get());
    }
    LockSupport.parkNanos(TimeUnit.SECONDS.toNanos(2));
    System.out.println(
        Try.of(() -> CircuitBreaker.decorateCheckedFunction(test, checkedFunction1).apply(false))
            .recover(t -> "方法降级")
            .get());
    System.out.println(
        Try.of(() -> CircuitBreaker.decorateCheckedFunction(test, checkedFunction1).apply(false))
